...
class QUIZ (db.Model):
    ...
    def json(self):
        return {"quizID": self.quizID, "CNo": self.CNo,
...
def find_quizzes_by_course(course_name, CNo):
    course_quizzes = QUIZ.query.filter_by(course_name=course_name,
                                          CNo=CNo).all()
    if course_quizzes:
        return jsonify(
            {
                "code": 200,
                "data": {
                    "courseQuizzes": [course_quiz.json() for course_quiz in course_quizzes]  # noqa: E501
                }
            }
        )
    return jsonify(
