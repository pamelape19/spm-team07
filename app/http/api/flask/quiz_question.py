        ...
        return jsonify(
            {
                "code": 200,
                "data": {
                    "quiz_question": [quiz_question.json() for quiz_question in quiz_question_list]  # noqa: E501
                }
            }
        )
    return jsonify(
...
def addNewQuiz(quizID):
    ...
    try:
        for qus in data["data"]:
            print(qus)
            new_quiz = QUIZ_QUESTION(question=qus["question"],
                                     questionNo=qus["question_no"],
                                     question_type=qus['question_type'],
                                     quizID=quizID)
            print(new_quiz)
            try:
